    ...
    return await new Promise((resolve, reject) => {
      reject(
        new BackendError(
          ErrorType.NotFound,
          "not found",
          `Could not find any entity of type "Pilot" matching: {\n    "id": ${id}\n}`
        )
      );
    });
  }
...
export const insert = async (pilot: Pilot): Promise<Pilot> => {
    ...
    return await new Promise((resolve, reject) => {
      return reject(
        new BackendError(
          ...
          "already exists",
          `${pilot.name.toLowerCase()} already exists in database`
        )
      );
    });
  }
      ...
      try {
        ...
        if (teamByName.length > 1) {
          return await new Promise((resolve, reject) => {
            return reject(
              new BackendError(
                ErrorType.MultipleMatch,
                "multiple match",
                `there is more than one team that matches the name of '${pilot.team.name}'`
              )
            );
          });
        }
          ...
          return await new Promise((resolve, reject) => {
            return reject(
              new BackendError(
                ErrorType.NotFound,
                "not found",
                `there is no team that match the name of ${pilot.team.name}`
              )
            );
          });
        }
  ...
  if (
    !(teamById === null || teamById === undefined) &&
    !(teamByName[0] === null || teamByName[0] === undefined)
  ) {
    if (
      teamByName[0].id !== teamById.id ||
      teamByName[0].name !== teamById.name
    ) {
      return await new Promise((resolve, reject) => {
        return reject(
          new BackendError(
            ...
            "id mismatch",
            `the id from request body is belong to '${
              (teamById as Team).name
            }' but in request the name was matching: '${teamByName[0].name}'`
          )
        );
      });
    }
    ...
    return await new Promise((resolve, reject) => {
      return reject(
        new BackendError(
          ErrorType.NotFound,
          "not found",
          `pilot with ${id} not exists in database`
        )
      );
    });
  }
...
export const update = async (id: number, pilot: Pilot): Promise<Pilot> => {
    ...
    return await new Promise((resolve, reject) => {
      return reject(
        new BackendError(
          ErrorType.AlreadyExists,
          "already exists",
          `pilot with the name of ${pilot.name.toLowerCase()} already exists in database`
        )
      );
    });
  }
    ...
    return await new Promise((resolve, reject) => {
      return reject(
        new BackendError(
          ErrorType.NotFound,
          "not found",
          `pilot with id of ${id} not exists in database`
        )
      );
    });
  }
  ...
  if (
    pilot.team !== null &&
    !(pilot.team.name === null || pilot.team.name === undefined)
  ) {
    try {
      teamByName = await teamRepo.get(pilot.team.name);
      if (teamByName.length > 1) {
      ...
      return await new Promise((resolve, reject) => {
        return reject(
          new BackendError(
            ...
            `the id from request body is belong to '${
              (teamById as Team).name
            }' but in request the name was natching: '${teamByName[0].name}'`
          )
        );
      });
    }
